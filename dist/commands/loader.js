...
function executeScript(script, context, filePath, ext) {
    if (ext === ".js") {
        try {
...
function loadCommandFromFile(filePath, source) {
    ...
    if (isScript) {
        try {
            ...
            const command = {
                ...
                handler: async (context) => executeScript(content, context, filePath, ext),
                source,
                filePath,
            };
            return command;
...
export function loadCommandsFromDir(dir, source = "filesystem") {
...
export function loadAllCommands(cwd = process.cwd()) {
    let total = 0;
