...
export const codeReviewCommand = {
    name: "code-review",
    ...
    handler: async (context) => {
        ...
        if (flags.result) {
            const result = reviewRunner.getResult(flags.result);
            if (!result)
                return { error: "Review result not found", exitCode: 1 };
...
export const securityReviewCommand = {
    ...
    handler: async (context) => {
        ...
        if (flags.result) {
            const result = reviewRunner.getResult(flags.result);
            if (!result)
                return { error: "Security review result not found", exitCode: 1 };
        ...
        if (secFindings.length === 0) {
            ...
            const bySeverity = (sev) => secFindings.filter(f => f.severity === sev);
