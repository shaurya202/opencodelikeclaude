...
export const loopCommand = {
    ...
    handler: async (context) => {
        ...
        if (flags.list) {
            ...
            if (loops.length === 0)
                return { output: "No loops running. Use /loop <prompt> to create one." };
            let output = "Loops:\n\n";
        ...
        if (flags.start) {
            const loop = scheduler.startLoop(flags.start);
            if (!loop)
                return { error: "Loop not found or already running", exitCode: 1 };
            return { output: `Started loop: ${loop.id.slice(0, 8)}` };
        }
        if (flags.pause) {
            const loop = scheduler.pauseLoop(flags.pause);
            if (!loop)
                return { error: "Loop not found or not running", exitCode: 1 };
            return { output: `Paused loop: ${loop.id.slice(0, 8)}` };
        }
        if (flags.cancel) {
            const loop = scheduler.cancelLoop(flags.cancel);
            if (!loop)
                return { error: "Loop not found", exitCode: 1 };
    ...
    handler: async (context) => {
        ...
        if (flags.list) {
            ...
            if (schedules.length === 0)
                return { output: "No schedules. Use /schedule <cron> <prompt> to create one." };
        ...
        if (flags.enable) {
            const s = scheduler.enableSchedule(flags.enable);
            if (!s)
                return { error: "Schedule not found", exitCode: 1 };
            return { output: `Enabled schedule: ${s.id.slice(0, 8)}` };
        }
        if (flags.disable) {
            const s = scheduler.disableSchedule(flags.disable);
            if (!s)
                return { error: "Schedule not found", exitCode: 1 };
            return { output: `Disabled schedule: ${s.id.slice(0, 8)}` };
        }
        if (flags.remove) {
            const found = scheduler.removeSchedule(flags.remove);
            if (!found)
                return { error: "Schedule not found", exitCode: 1 };
...
export const ralphLoopCommand = {
    ...
    handler: async (context) => {
        ...
        if (flags.cancel) {
            const found = scheduler.removeRalphLoop(flags.cancel);
