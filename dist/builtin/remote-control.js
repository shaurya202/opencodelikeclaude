    ...
    handler: async (context) => {
        ...
        if (flags.sessions) {
            ...
            if (sessions.length === 0)
                return { output: "No connected sessions." };
        ...
        if (flags.disconnect) {
            const found = remoteControlServer.disconnectSession(flags.disconnect);
            return { output: found ? "Session disconnected" : "Session not found" };
        }
        if (flags.broadcast) {
            remoteControlServer.broadcast(flags.broadcast);
            return { output: "Message broadcast to all sessions" };
        }
        if (flags.start) {
            remoteControlServer.updateConfig({ enabled: true, port: Number(flags.port) || config.remote.control.port, host: flags.host || config.remote.control.host });
            await remoteControlServer.start();
            try {
                saveConfig(cwd, { ...config, remote: { ...config.remote, control: { ...config.remote.control, enabled: true } } });
            }
            catch { /* ignore */ }
        ...
        if (flags.port) {
            ...
            try {
                saveConfig(cwd, { ...config, remote: { ...config.remote, control: { ...config.remote.control, port: Number(flags.port) } } });
            }
            catch { /* ignore */ }
