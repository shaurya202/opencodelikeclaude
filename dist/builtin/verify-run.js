...
export const verifyCommand = {
    ...
    handler: async (context) => {
        const { flags } = context;
        if (flags.result) {
            const result = verifyRunner.getResult(flags.result);
            if (!result)
                return { error: "Verify result not found", exitCode: 1 };
        ...
        const result = await verifyRunner.verify(target, {
            build: flags.build ?? all ?? true,
            test: flags.test ?? all ?? true,
            lint: flags.lint ?? all ?? true,
...
export const runCommand = {
    ...
    handler: async (context) => {
        ...
        const command = args.join(" ") || flags.args || "npm start";
        ...
        output += `\nCommand: ${command}\n`;
        if (flags.detach)
            output += "Mode: detached (background)\n";
