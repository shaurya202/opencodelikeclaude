...
export const insightsCommand = {
    ...
    handler: async (context) => {
        ...
        if (flags.list) {
            ...
            if (reports.length === 0)
                return { output: "No insight reports. Use /insights --generate <sessionId> to create one." };
        ...
        if (flags.get) {
            const report = insightsAnalyzer.getReport(flags.get);
            if (!report)
                return { error: "Report not found", exitCode: 1 };
        ...
        if (flags.generate) {
            const report = insightsAnalyzer.generateReport(flags.generate);
