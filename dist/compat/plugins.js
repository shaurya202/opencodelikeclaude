...
export function discoverClaudePlugins(cwd = process.cwd()) {
    const plugins = [];
...
export function loadClaudePlugins(cwd = process.cwd()) {
...
export function getPluginPaths(cwd = process.cwd()) {
    return CLAUDE_PLUGIN_DIRS.map(dir => dir.startsWith("~/") ? expandHome(dir) : join(cwd, dir)).filter(p => existsSync(p));
}
