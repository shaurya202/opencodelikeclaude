...
export const batchCommand: CommandDefinition = {
  ...
  handler: async (context: CommandContext): Promise<CommandResult> => {
    ...
    if (flags.list) {
      ...
      if (batches.length === 0) return { output: "No batches. Use /batch <task> --units <count> to create one." };
      let output = "Batches:\n\n";
    ...
    if (flags.get) {
      const batch = batchRunner.getBatch(flags.get as string);
      if (!batch) return { error: "Batch not found", exitCode: 1 };
      ...
      for (const job of batch.jobs) {
        ...
        if (job.result) output += `    Result: ${job.result.slice(0, 100)}${job.result.length > 100 ? "..." : ""}\n`;
    ...
    if (flags.start) {
      const batch = batchRunner.startBatch(flags.start as string);
      if (!batch) return { error: "Batch not found or already started", exitCode: 1 };
      return { output: `Started batch: ${batch.name} (${batch.id.slice(0, 8)}) with ${batch.parallel} parallel jobs` };
    }

    if (flags.cancel) {
      const batch = batchRunner.cancelBatch(flags.cancel as string);
      if (!batch) return { error: "Batch not found", exitCode: 1 };
      return { output: `Cancelled batch: ${batch.name}` };
    }

    if (flags.remove) {
      const found = batchRunner.removeBatch(flags.remove as string);
    ...
    const batch = batchRunner.createBatch(
      (flags.name as string) || `batch-${Date.now()}`,
      task,
      unitsList
    );
