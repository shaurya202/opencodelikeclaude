...
export const teleportCommand: CommandDefinition = {
  name: "teleport",
  ...
  handler: async (context: CommandContext): Promise<CommandResult> => {
    ...
    if (flags.list) {
      ...
      if (sessions.length === 0) return { output: "No teleported sessions. Use /teleport <url> to import one." };
    ...
    if (flags.get) {
      const session = teleportManager.getSession(flags.get as string);
      if (!session) return { error: "Session not found", exitCode: 1 };
      return {
    ...
    if (flags.remove) {
      const found = teleportManager.removeSession(flags.remove as string);
    ...
    if (flags.fetch) {
      const result = await teleportManager.fetchFromWeb(flags.fetch as string);
      return { output: `Fetched: ${result.title}\n\n${result.content.slice(0, 1000)}${result.content.length > 1000 ? "..." : ""}` };
    }

    if (flags.push) {
      const [sessionId, ...contentParts] = (flags.push as string).split(" ");
    ...
    const session = await teleportManager.importSession(url, flags.source as string || undefined);
