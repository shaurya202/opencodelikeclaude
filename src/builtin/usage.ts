...
import { PlanTier } from "../cost/types";
...
export const usageCommand: CommandDefinition = {
  ...
  handler: async (context: CommandContext): Promise<CommandResult> => {
    ...
    if (flags.plan) {
      const tier = flags.plan as PlanTier;
      const valid: PlanTier[] = ["free", "pro", "max", "team", "enterprise"];
    ...
    if (flags.alerts) {
      ...
      if (alerts.length === 0) return { output: "No cost alerts configured." };
