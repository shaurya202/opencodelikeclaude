...
export const worktreeCommand: CommandDefinition = {
  ...
  handler: async (context: CommandContext): Promise<CommandResult> => {
    ...
    if (flags.list) {
      ...
      if (all.length === 0) return { output: "No worktrees. Use /worktree <name> to create one." };
      let output = "Worktrees:\n\n";
      for (const w of all) {
        output += `  [${w.status}] ${w.name} → ${w.branch}\n`;
        output += `    Path: ${w.path}  Created: ${new Date(w.createdAt).toLocaleDateString()}\n`;
        if (w.prUrl) output += `    PR: ${w.prUrl}\n`;
      }
      return { output };
    }

    if (flags.get) {
      const wt = worktreeManager.get(flags.get as string);
      if (!wt) return { error: "Worktree not found", exitCode: 1 };
    ...
    if (existing) return { error: `Worktree "${name}" already exists`, exitCode: 1 };

    const wt = await worktreeManager.create(name, { baseBranch: flags.base as string });
