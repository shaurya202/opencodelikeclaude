...
export function exportSession(sessionId: string, format: "json" | "markdown" = "json"): string | null {
...
export function importSession(filepath: string): string | null {
...
export function listExports(): string[] {
  ...
  return files.filter((f: string) => f.endsWith(".json") || f.endsWith(".md"));
}